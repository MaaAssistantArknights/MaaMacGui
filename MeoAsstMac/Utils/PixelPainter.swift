...
struct PixelPainter {
    ...
    func convert(image: CGImage, in bounds: CGRect) -> PixelPainting? {
        ...
        if config.noiseReductionEnabled {
            ...
            filter.inputImage = ciImage
            filter.noiseLevel = Float(config.noiseLevel)
...
struct PixelPainting: Hashable {
    ...
    private func pixels(fillBlankWithWhite: Bool) -> [(xy: SIMD2<Int>, rgb: SIMD3<UInt8>)] {
        let columns = bitmap.count / rows
        let width = fillBlankWithWhite ? Int(bounds.width) : columns
        let height = fillBlankWithWhite ? Int(bounds.height) : rows
        ...
        if fillBlankWithWhite {
            let rect = CGRect(x: offsetX, y: offsetY, width: columns, height: rows)
            for y in Int(bounds.minY)..<Int(bounds.maxY) {
                for x in Int(bounds.minX)..<Int(bounds.maxX) {
                    let point = CGPoint(x: x, y: y)
                    if !rect.contains(point) {
                        let xy = SIMD2(x, y)
                        result.append((xy, white))
                    }
                }
            }
        }
    ...
    func cells(fillBlankWithWhite: Bool) -> [(xy: SIMD2<Int>, Color)] {
        pixels(fillBlankWithWhite: fillBlankWithWhite).compactMap { (xy, rgb) in
    ...
    func colorPoints(fillBlankWithWhite: Bool) -> [Int: [SIMD2<Int>]] {
        var result = [Int: [SIMD2<Int>]]()
        for pixel in pixels(fillBlankWithWhite: fillBlankWithWhite) {
