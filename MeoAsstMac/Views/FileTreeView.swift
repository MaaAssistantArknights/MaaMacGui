...
struct FileTreeRoot<Item: FileTreeItem, Header: View, Label: View>: View {
        ...
        if item.url.isDirectory {
            if let $children = Binding($item.children) {
                ForEach($children) { $i in
                    FileTreeNode(item: $i, tracker: tracker, header: header, label: label)
                }
            }
        } else {
            label(item)
...
struct FileTreeNode<Item: FileTreeItem, Header: View, Label: View>: View {
    @Binding var item: Item
    let tracker: FileTreeTracker?
...
#Preview {
    ...
    List(selection: $selection) {
        FileTreeRoot(item: $item, tracker: tracker) {
            ...
            Text($0.url.deletingPathExtension().lastPathComponent)
        }
    }
